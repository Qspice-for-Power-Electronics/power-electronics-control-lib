...
class ProjectConfig:
        ...
        for module_type in self.config["modules"]:
            for component_name, component in self.config["modules"][module_type]["components"].items():
                if component["headers"]:
                    # Convert forward slashes to OS-appropriate separators
                    include_paths.append(str(self.root_path / Path(component["path"])))
    ...
    def print_summary(self):
        ...
        # Handle cases where author is in metadata or missing
        author = self.config['project'].get('author', 
                 self.config['project'].get('_metadata', {}).get('author', 'Unknown'))
        ...
        for module_type in self.config["modules"]:
            print(f"  {module_type}:")
            for comp_name, comp_config in self.config["modules"][module_type]["components"].items():
                sources_count = len(comp_config['sources']) if comp_config['sources'] else 0
                headers_count = len(comp_config['headers']) if comp_config['headers'] else 0
                print(f"    - {comp_name}: {sources_count} sources, {headers_count} headers")
