...
{
    float V_ac = data[0].f;      // input
    float I_ac = data[1].f;      // input
    float Itank_ac = data[2].f;  // input
    float In1 = data[3].f;       // input
    float In2 = data[4].f;       // input
    float In3 = data[5].f;       // input
    float In4 = data[6].f;       // input
    float In5 = data[7].f;       // input
    float In6 = data[8].f;       // input
    float In7 = data[9].f;       // input
    float Itank_dc = data[10].f; // input
    float V_dc = data[11].f;     // input
    float I_dc = data[12].f;     // input
    ...
    float &Out21 = data[45].f;   // output
    float &Out22 = data[46].f;   // output
    float &Out23 = data[47].f;   // output
    float &Out24 = data[48].f;   // output
    float &Out25 = data[49].f;   // output
    float &Out26 = data[50].f;   // output
    float &Out27 = data[51].f;   // output
    float &Out28 = data[52].f;   // output
    ...
    if (!mod_initialized)
    {
        PwmParams pwm_params = {10e-6f, 0, 15.0f}; // Ts, carrier_select, gate_on_voltage
        pwm_module_init(&mod, &pwm_params);
        IirParams lpf_params = {1e-4f, 100.0f, 0, 0.0f}; // Ts, fc, type=lowpass, a=auto
